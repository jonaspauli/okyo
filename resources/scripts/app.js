...
var textband = $('.textband');

// Support small text - copy to fill screen width
if (textband.find('.scrolling-text').outerWidth() < $(window).width()) {
    var windowToScrolltextRatio = Math.round($(window).width() / textband.find('.scrolling-text').outerWidth()),
        scrollTextContent = textband.find('.scrolling-text .scrolling-text-content').text(),
        newScrollText = '';
    for (var i = 0; i < windowToScrolltextRatio; i++) {
        newScrollText += ' ' + scrollTextContent;
    }
    textband.find('.scrolling-text .scrolling-text-content').text(newScrollText);
}

// Init variables and config
var scrollingText = textband.find('.scrolling-text'),
    ...
    transformDirection = textband.hasClass('left-to-right') ? -1 : 1,
    transformSpeed = 200;

// Read transform speed
if (textband.attr('speed')) {
    transformSpeed = textband.attr('speed');
}

// Make scrolling text copy for scrolling infinity
textband.append(scrollingText.clone().addClass('scrolling-text-copy'));
textband.find('.scrolling-text').css({'position': 'absolute', 'left': 0});
textband.css('height', scrollingTextHeight);

var getActiveScrollingText = function(direction) {
    var firstScrollingText = textband.find('.scrolling-text:nth-child(1)');
    var secondScrollingText = textband.find('.scrolling-text:nth-child(2)');

    var firstScrollingTextLeft = parseInt(textband.find('.scrolling-text:nth-child(1)').css("left"), 10);
    var secondScrollingTextLeft = parseInt(textband.find('.scrolling-text:nth-child(2)').css("left"), 10);
